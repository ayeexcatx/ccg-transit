...
* **Upcoming Dispatches:** Shows tomorrow’s truck counts (day \+ night)  
* **Friday logic:**  
  * If today is Friday, show **Monday’s** day/night totals instead of tomorrow  
  * If Sunday dispatches exist, display an indicator (e.g., asterisk/note)
...
* When clicked, it sends a **modal prompt to all users** instructing them to refresh/update the app  
* Users must acknowledge (click OK), which triggers the refresh  
* **Version control logic:** The system maintains a version number; when clicked, the current version is updated  
* **Client-side behavior:** Each user session compares its current version to the latest version  
* **Mismatch handling:** If versions do not match, the user is prompted to refresh the app, ensuring all users are on the latest version
...
* **Available Workspaces**  
  ...
  * Intended for specific users who need both roles  
* **SMS Enabled Toggle**  
...
* Notification states:  
  ...
  * Read → completed or acknowledged  
* Sync behavior:  
...
* Allows company owner to:  
  ...
  * Select which contact info is used for SMS
